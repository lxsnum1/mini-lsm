...
impl LsmStorageInner {
        ...
        'outer: while iter.is_valid() {
            ...
            if iter.key().ts() <= watermark {
                ...
                if !compaction_filters.is_empty() {
                    let mut a = compaction_filters.iter();
                    while let Some(CompactionFilter::Prefix(x)) = a.next() {
            ...
            if builder_inner.estimated_size() >= self.options.target_sst_size && !same_last_key {
    ...
    fn compact(&self, task: &CompactionTask) -> Result<Vec<Arc<SsTable>>> {
        ...
        match task {
            CompactionTask::ForceFullCompaction {
                ...
                let mut l0_ietrs = Vec::with_capacity(l0_sstables.len());
                for id in l0_sstables {
                    l0_ietrs.push(Box::new(SsTableIterator::create_and_seek_to_first(
                ...
                self.compact_sst_from_iter(iter, task.compact_to_bottom_level())
            }
            ...
            | CompactionTask::Leveled(LeveledCompactionTask {
                ...
                Some(_) => {
                    let mut upper_ssts = Vec::with_capacity(lower_level_sst_ids.len());
            ...
            CompactionTask::Tiered(TieredCompactionTask { tiers, .. }) => {
                ...
                for (_, tier_sst_ids) in tiers {
                    let mut sst = Vec::with_capacity(tier_sst_ids.len());
                    for id in tier_sst_ids {
                        sst.push(snapshot.sstables[id].clone())
                    }
                    iters.push(Box::new(SstConcatIterator::create_and_seek_to_first(sst)?));
                }
    ...
    pub fn force_full_compaction(&self) -> Result<()> {
        ...
        {
            let _stat_lock = self.state_lock.lock();
            let mut state = self.state.read().as_ref().clone();

            for sst_id in l0_sstables.iter().chain(l1_sstables.iter()) {
                assert!(state.sstables.remove(sst_id).is_some());
            }

            for new_sst in sstables {
                ids.push(new_sst.sst_id());
                assert!(state.sstables.insert(new_sst.sst_id(), new_sst).is_none());
            }
    ...
    fn trigger_flush(&self) -> Result<()> {
        let imm_len = self.state.read().imm_memtables.len();
        if imm_len >= self.options.num_memtable_limit {
