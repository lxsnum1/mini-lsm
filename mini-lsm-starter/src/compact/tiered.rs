...
impl TieredCompactionController {
    ...
    pub fn generate_compaction_task(
        ...
        for l in 0..snapshot.levels.len() - 1 {
            size += snapshot.levels[l].1.len();
        }
        let space_amp_ratio = (size as f64) / (snapshot.levels.last().unwrap().1.len() as f64);
        if space_amp_ratio >= (self.options.max_size_amplification_percent * 100) as f64 {
            println!(
        ...
        let size_ratio_trigger = 1.0 + self.options.size_ratio as f64 / 100.0;
        // compaction triggered by size ratio
        let mut previous = 0.0;
        for id in 0..snapshot.levels.len() {
            let size = snapshot.levels[id].1.len() as f64;
            let current_size_ratio = previous / size;
